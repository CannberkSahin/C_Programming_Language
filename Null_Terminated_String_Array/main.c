...
int main()
{
    const char *p[] = {
        ...
        "emre","sinem","teoman","ýrmak","cihat","adem","bilgin","muzaffer","suphi","pelin",
        ...
        "ilayda","gokce","merve","buket","azra",NULL};

        //Array elements are printed on the screen one under the other
        int i=0;

        while(p[i] != NULL){ //while(p[i])
            puts(p[i++]);
        }
}
